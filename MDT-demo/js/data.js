...
const diseaseIndicators = {
    '肝癌': ['AFP', 'PIVKA-II', 'ALT', 'AST', 'TBIL', 'ALB', 'PT'],
    '肝细胞癌': ['AFP', 'PIVKA-II', 'ALT', 'AST', 'TBIL', 'ALB', 'PT'],
    '肝恶性肿瘤': ['AFP', 'PIVKA-II', 'ALT', 'AST', 'TBIL', 'ALB', 'PT'],
...
// ── Auto-attach observations to timeline events from indicators ──────────
(function attachTimelineObservations() {
    mockPatients.forEach(function (patient) {
        const dateMap = {};
        Object.entries(patient.indicators || {}).forEach(function ([name, points]) {
            points.forEach(function (pt) {
                if (!dateMap[pt.date]) dateMap[pt.date] = [];
                dateMap[pt.date].push({ name: name, value: pt.value });
            });
        });
        (patient.timeline || []).forEach(function (event) {
            if (!event.observations || event.observations.length === 0)
                event.observations = dateMap[event.date] || [];
        });
    });
})();

const indicatorRanges = {
    'AFP': { min: 0, max: 16.8, unit: 'ng/mL' },
    'PIVKA-II': { min: 0, max: 40, unit: 'ng/mL' },
