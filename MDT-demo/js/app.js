...
function mapEventType(type) {
    const map = {
        ...
        'imaging':         'imaging',
        'procedure':       'treatment',
        'procedure_surgery': 'surgery',
        'surgery':         'surgery',
        'diagnosis':       'diagnosis',
        'pathology_report': 'evaluation',
        'medication':      'treatment',
        'mdt_record':      'mdt',
        'document':        'evaluation',
        'followup_outcome': 'followup',
    };
    return map[type] || 'evaluation';
}

/**
 * Deprecated: all data now comes from uiProjection directly.
 * Kept as a no-op to avoid reference errors.
 */
function enrichFromBasic(_patient) {}

/**
 * 从 js/bundle-data.js (BUNDLE_DATA) 加载 timeline/indicators，
 * 从 js/bundle-basic.js (BUNDLE_BASIC) 加载患者基本信息。
 * BUNDLE_BASIC.patientData 是患者身份与病史的唯一数据源。
 */
async function loadPatientData() {
    try {
        const bundle = (typeof BUNDLE_DATA !== 'undefined') ? BUNDLE_DATA : null;
        if (!bundle) throw new Error('BUNDLE_DATA global not found – is js/bundle-data.js loaded?');

        const ui = bundle.extractionResult?.uiProjection || {};

        // ── Timeline ────────────────────────────────────────────────────────
        // uiProjection.timeline is already LLM-deduplicated and has observations attached.
        ...
        // ── Indicators ──────────────────────────────────────────────────────
        const indicators = {};
        Object.entries(ui.indicators || {}).forEach(([key, points]) => {
            if (!Array.isArray(points)) return;
            indicators[key] = points
                .filter(pt => pt && pt.date && pt.value !== undefined)
                .sort((a, b) => new Date(a.date) - new Date(b.date));
        });

        // ── Patient identity & history from BUNDLE_BASIC.patientData ────────
        const pd = (typeof BUNDLE_BASIC !== 'undefined') ? (BUNDLE_BASIC.patientData || {}) : {};

        const name    = pd.name  || '未知';
        const gender  = pd.sex   || '未知';
        const age     = (typeof pd.age === 'number') ? pd.age : 0;
        const disease = pd.primaryDiagnosis || '待补充';
        const stage   = pd.stage  || null;
        const complications = Array.isArray(pd.comorbidities) ? pd.comorbidities : [];

        // ── Auto-group by disease ────────────────────────────────────────────
        let group = '';
        const dl = disease.toLowerCase();
        if (dl.includes('肝'))        group = 'liver';
        else if (dl.includes('肺'))   group = 'lung';
        else if (dl.includes('淋巴')) group = 'lymphoma';
        else if (dl.includes('胃'))   group = 'gastric';

        const realPatient = {
            id:           ui.id || bundle.extractionResult?.extractionId || 'REAL001',
            name,
            gender,
            age,
            disease,
            stage,
            complications,
            treatment:    ui.treatment || '综合治疗',
            group,
            lastUpdate:   ui.lastUpdate || '',
            timeline,
            indicators,
            basicInfo: {
                chiefComplaint:  pd.chiefComplaint  || '',
                presentHistory:  pd.presentHistory  || '',
                pastHistory:     pd.pastHistory     || '',
                familyHistory:   pd.familyHistory   || '暂无记录',
                personalHistory: pd.personalHistory || '暂无记录',
            },
        };

        mockPatients.length = 0;
        mockPatients.push(realPatient);

        console.log(
            '[智汇MDT] Loaded – patient:', realPatient.name,
            '| timeline:', timeline.length,
            '| indicators:', Object.keys(indicators).join(', ') || '(none)'
        );
    } catch (err) {
        console.warn('[智汇MDT] Could not load BUNDLE_DATA, keeping mock data:', err.message);
    }
}
...
function renderEventDetail(index) {
    ...
    // Use only the per-event bundled observations (do not fall back to date-level scanning
    // which would show the same indicators across all events sharing the same date).
    const indicators = [];
    const _eventObs = Array.isArray(event.observations) ? event.observations : [];
    _eventObs.forEach(obs => {
        const range = indicatorRanges[obs.name];
        const isAbnormal = range && (obs.value < range.min || obs.value > range.max);
        indicators.push({ name: obs.name, value: obs.value, unit: indicatorUnits[obs.name] || '', isAbnormal, range });
    });
