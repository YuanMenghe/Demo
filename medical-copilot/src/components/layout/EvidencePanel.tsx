...
import { FileText, Calendar, MessageSquare, ExternalLink, X } from 'lucide-react';
import { Evidence, MOCK_EVIDENCE } from '@/data/mock';
...
export function EvidencePanel({ evidenceId, onClose }: EvidencePanelProps) {
  const evidence = evidenceId ? MOCK_EVIDENCE[evidenceId] : null;
  ...
  return (
    <div className="w-80 border-l border-slate-200 bg-white h-[calc(100vh-3.5rem)] sticky top-14 flex flex-col shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)]">
      <div className="p-4 border-b border-slate-100 flex items-center justify-between bg-slate-50/50">
        ...
        {/* Mobile close button if needed, though this is desktop-first design */}
      </div>
          ...
          {evidence ? (
            ...
            >
              ...
              <div className="space-y-3">
                <h5 className="text-xs font-semibold text-slate-500 uppercase tracking-wider">Source Metadata</h5>
            ...
            <div className="h-full flex flex-col items-center justify-center text-slate-400 text-center p-4">
              ...
              <p className="text-sm">Click on any highlighted text or citation to view its source evidence.</p>
            </div>
          )}
        </AnimatePresence>
      </div>
